...
class SpikeCuber(object):
    ...
    def __init__(self):
        ...
        self.colorsensor = hub.port.E
        while self.colorsensor.device is None:
            utime.sleep(0.1)
        self.colorsensor.device.mode(5)
    ...
    def scan_side(self, side_name, scan_order):
        ...
        def rgb():
            (red, green, blue, _intensity) = self.colorsensor.device.get()
        ...
        for x in range(9):
            ...
            if x == 0:
                self.colorarm_middle()
                self.rgb_values[square_index] = rgb()
